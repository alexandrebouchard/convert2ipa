...
public class CounterMap<K, V> implements java.io.Serializable {
	...
	/**
	 * Clears all content in this CounterMap
	 *
	 */
	...
	/**
	 * Increments the count for all (key, value) pairs in the countermap
	 * argument.
	 */
	public void incrementAll(CounterMap<K, V> map) {
		for (K key : map.keySet()) {
			for (V value : map.getCounter(key).keySet()) {
				incrementCount(key, value, map.getCount(key, value));
			}
		}
	}
